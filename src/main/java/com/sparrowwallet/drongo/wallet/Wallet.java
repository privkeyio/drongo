...
public class Wallet extends Persistable implements Comparable<Wallet> {
    ...
    static void retainThreshold(Map<ECKey, TransactionSignature> pubKeySignatures, int threshold, PSBTInput psbtInput) {
        ...
        //copy of it: both are then kept, and the witness carries the one signature twice. CHECKMULTISIG walks the
        //signatures against the keys in one pass and never matches one of them twice, so the second fails and this
        //does not spend. Where nothing can be checked this finds none preferred and falls back to key order, which is
        //what it did before.
